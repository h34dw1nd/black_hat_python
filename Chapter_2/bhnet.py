...
def usage():
    ...
    print "-u --upload=destination  - upon receing connection upload a file write to [destination]"
    print 
    print 
    print "Examples: "
...
def main():
    ...
    # 是进行监听还是从标准输入发送数据?
    if not listen and len(target) and target > 0:

        # 从命令行读取缓冲区数据
        # 这里将阻塞，所以在不向命令行发送数据时发送ctrl+D
